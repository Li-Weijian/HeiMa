...
$.extend(Datepick.prototype, {
	...
	_mainDivId: 'datepicker-div', // The ID of the main datepicker division
	_inlineClass: 'datepicker-inline', // The name of the inline marker class
	_appendClass: 'datepicker-append', // The name of the append marker class
	_triggerClass: 'datepicker-trigger', // The name of the trigger marker class
	_dialogClass: 'datepicker-dialog', // The name of the dialog marker class
	_promptClass: 'datepicker-prompt', // The name of the dialog prompt marker class
	_disableClass: 'datepicker-disabled', // The name of the disabled covering marker class
	_oneMonthClass: 'datepicker-one-month', // The name of the single month marker class
	_unselectableClass: 'datepicker-unselectable', // The name of the unselectable cell marker class
	_currentClass: 'datepicker-current-day', // The name of the current day marker class
	_dayOverClass: 'datepicker-days-cell-over', // The name of the day hover marker class
	_weekOverClass: 'datepicker-week-over', // The name of the week hover marker class
	_coverClass: 'datepicker-cover', // The name of the IE6- iframe marker class
	...
	_selectDay: function(id, year, month, td) {
		...
		if (rangeSelect) {
			...
			if (inst.stayOpen) {
				$('.datepicker td', inst.dpDiv).removeClass(this._currentClass);
	...
	_generateHTML: function(inst) {
		...
		var clear = (this._get(inst, 'mandatory') ? '' :
			'<div class="datepicker-clear"><a onclick="jQuery.datepicker._clearDate(\'#' + inst.id + '\');"' +
			this._addStatus(showStatus, inst.id, this._get(inst, 'clearStatus'), initStatus) + '>' +
			this._get(inst, 'clearText') + '</a></div>');
		var controls = '<div class="datepicker-control">' + (isRTL ? '' : clear) +
			'<div class="datepicker-close"><a onclick="jQuery.datepicker._hideDatepick();"' +
		...
		prevBigText = (!navigationAsDateFormat ? prevBigText : this.formatDate(prevBigText,
			...
			this._getFormatConfig(inst)));
		var prev = '<div class="datepicker-prev">' + (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ?
		...
		nextBigText = (!navigationAsDateFormat ? nextBigText : this.formatDate(nextBigText,
			this._daylightSavingAdjust(new Date(drawYear, drawMonth + stepBigMonths, 1)),
			this._getFormatConfig(inst)));
		var next = '<div class="datepicker-next">' + (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
			'<a onclick="jQuery.datepicker._adjustDate(\'#' + inst.id + '\', +' + stepMonths + ', \'M\');"' +
		...
		var html = (closeAtTop && !inst.inline ? controls : '') +
			'<div class="datepicker-links">' + (isRTL ? next : prev) +
			(this._isInRange(inst, gotoDate) ? '<div class="datepicker-current">' +
		...
		for (var row = 0; row < numMonths[0]; row++)
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dow = 0; dow < 7; dow++) { // days of the week
					...
					html += '<th' + ((dow + firstDay + 6) % 7 >= 5 ? ' class="datepicker-week-end-cell"' : '') + '>' +
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					html += '<tr class="datepicker-days-row">' +
						(showWeeks ? '<td class="datepicker-week-col"' +
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						html += '<td class="datepicker-days-cell' +
							((dow + firstDay + 6) % 7 >= 5 ? ' datepicker-week-end-cell' : '') + // highlight weekends
							...
							(!showStatus || empty ? '' : 'jQuery(\'#datepicker-status-' + inst.id + '\').html(\'' + // show status
							...
							(!showStatus || empty ? '' : 'jQuery(\'#datepicker-status-' + inst.id + '\').html(\'' + initStatus + '\');') +
		...
		html += (showStatus ? '<div style="clear: both;"></div><div id="datepicker-status-' + inst.id +
			'" class="datepicker-status">' + initStatus + '</div>' : '') +
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		if (secondary || !changeMonth)
			...
			monthHtml += '<select class="datepicker-new-month" ' +
		...
		if (secondary || !changeYear)
			...
			html += '<select class="datepicker-new-year" ' +
	...
	_addStatus: function(showStatus, id, text, initStatus) {
		return (showStatus ? ' onmouseover="jQuery(\'#datepicker-status-' + id +
			'\').html(\'' + (text || initStatus) + '\');" ' +
			'onmouseout="jQuery(\'#datepicker-status-' + id +
			'\').html(\'' + initStatus + '\');"' : '');
	},
